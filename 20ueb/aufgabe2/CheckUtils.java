...
public class CheckUtils {
    ...
    public static void checkBuchung(int menge) {
        if (menge <= 0) {
            throw new IllegalArgumentException("Menge darf nicht negativ, oder 0 sein");
        }
    }

    public static void checkAbgangBuchung(int bestand, int menge) {
        if (bestand - menge < 0) {
            throw new IllegalArgumentException("Menge darf nicht größer als Bestand sein," 
                    + " sonst ist der Bestand negativ");
        }
    }
     ...
     */
    public static void checkIfInLager(int artikelNr, Map<Integer, Artikel> lagerMap) {
        for(Map.Entry<Integer, Artikel> entry : lagerMap.entrySet()){
            if (artikelNr == entry.getKey()){
     ...
     */
    public static void checkIfNotInLager(int artikelNr, Map<Integer, Artikel> lagerMap) {
        if (!lagerMap.containsKey(artikelNr)){
            throw new IllegalArgumentException("Die Artikelnummer " + artikelNr + " ist nicht vorhanden.");
        }
    }

    public static void checkIfLagerIsEmpty(Map<Integer, Artikel> lagerMap) {
        if (lagerMap.isEmpty()) {
            throw new IllegalArgumentException("Das Lager ist leer. Sie muessen erstmal Artikel anlegen.");
        }
    }
